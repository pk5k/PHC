...
if (document.componentStyleMap == undefined){document.componentStyleMap = []};
...
document.importFontList = function()
{
  if (document.importedFontsList != undefined)
  {
    return document.importedFontsList;
  }

  document.importedFontsList = [];

  // create constructed stylesheet list for fonts once to add it to the adopt stylesheets list of the shadowDom
  document.head.querySelectorAll('.container-font').forEach((f) => {
    let css = new CSSStyleSheet();

    for (let i in f.sheet.cssRules) 
    {
      let rule = f.sheet.cssRules[i];

      if (rule.cssText != undefined)
      {
        css.insertRule(rule.cssText);
      }
    }

    document.importedFontsList.push(css);
  });

  return document.importedFontsList;
};

document.exportStylesheet = function(css, context_id)
{
  if (context_id == 'global' || context_id == '' || !context_id)
  {
    return;
  }

  let context = document.getElementById(context_id);

  if (!context.hasAttribute('data-global') || context.getAttribute('data-global') != 'true')
  {
    context = null;
    context_id = null;
    css = null;
  ...
  let es = document.createElement('style');
  es.setAttribute('type', 'text/css');
  es.classList.add('style-global');
  let body = '';

  for (let i in css.cssRules)
  {
    let rule = css.cssRules[i];
...
document.registerComponentController = function(hcfqn, controller_class, as_element, element_options)
{
  ...
  if (target == null)
  {
    ...
    if (controller_class.prototype != undefined)
    {
...
document.cloneRenderContext = function(which, register_to_fqn)
{
    ...
    let base_styles = document.componentStyleMap[which];
    let cloned = [];

    if (base_styles == undefined)
    {
      base_styles = [];
    }

    document.componentStyleMap[dyn_cc] = base_styles.map((s) => s);// map clones the entry instead of passing a reference by = which will add new stylesheets to other cloned render contexts
    return new_cc;
  };

// Constructed CSSStyleSheet polyfill:
(function () {
    'use strict';
