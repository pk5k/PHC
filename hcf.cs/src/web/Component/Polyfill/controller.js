    ...
    var loop = function loop(nodes, added, removed, connected, pass) {
      for (var i = 0, length = nodes.length; i < length; i++) {
        ...
        if (pass || QSA$1 in node) {
          ...
          if (window['hcf'] != undefined && node instanceof hcf.web.Component) {
            if (node instanceof hcf.web.Page && !node.hasAttribute('wc-observer-fallback-setup')) {
              node.classList.add('wc-observer-fallback-setup');
              observer.add(node.shadowRoot);

              loop(node.shadowRoot[QSA$1]('*'), added, removed, connected, TRUE);
            }
          } else if (window['hcf'] == undefined) {
            throw 'hcf not ready';
          }
        }
      }
