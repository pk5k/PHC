...
class LoggerConfigurationAdapterINI implements LoggerConfigurationAdapter {
	 ...
	 */
	private function load($url, $from_string) {
		$properties = false;

		if ($from_string) {
			$properties = @parse_ini_string($url, true);
		} else {
			if (!file_exists($url)) {
	...
	*
	* @param string $path Path to the config file. Or the config itself, if $from_string is true
	* @param bool Determinates, if $path is a file or holds the config
	* @throws LoggerException If the file cannot be loaded or parsed.
	*/
	public function convert($path, $from_string = false) {
		// Load the configuration
		$properties = $this->load($path, $from_string);
