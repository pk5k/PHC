...
class Worker {
    ...
    public static function script() {
        $js = "onmessage=function(e)
{var data=e.data;var http_request=false;var timeout=4000;var info=false;var http_request=new XMLHttpRequest();timeout=(!isNaN(data.overwrites.timeout))?data.overwrites.timeout:timeout;http_request.ontimeout=function()
...
var send_data=argsToFormData(data.args,data.files);http_request.open('POST',data._.route,true);http_request.setRequestHeader(data._.header.action,data._.action);http_request.setRequestHeader(data._.header.target,data._.target);http_request.timeout=timeout;http_request.send(send_data);function returnToHost(result,code,response_data)
{var ret_obj={token:data._.token};switch(result)
{case'timeout':case'error':case'success':ret_obj.result=result;ret_obj.code=code;ret_obj.data=response_data;break;default:throw'Invalid result \"'+result+'\" given.';}
postMessage(ret_obj);}
