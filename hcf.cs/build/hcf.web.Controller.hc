<?php #HYPERCELL hcf.web.Controller - BUILD 22.03.23#14
namespace hcf\web;
class Controller {
...
let controller=new Promise((resolve,reject)=>{let elem=document.createElement('script');elem.setAttribute('src',base_href+'?!=-script'+component+version);let cbres=(e)=>{resolve(e);e.target.removeEventListener('load',cbres)};let cbrej=(e)=>{reject(e);e.target.removeEventListener('error',cbrej)};elem.addEventListener('load',cbres);elem.addEventListener('error',cbrej);document.head.appendChild(elem);elem=null;});let template=new Promise((resolve,reject)=>{let elem=document.createElement('script');elem.setAttribute('src',base_href+'?!=-template'+component+context+version);let cbres=(e)=>{resolve(e);e.target.removeEventListener('load',cbres)};let cbrej=(e)=>{reject(e);e.target.removeEventListener('error',cbrej)};elem.addEventListener('load',cbres);elem.addEventListener('error',cbrej);document.head.appendChild(elem);});let style=new Promise((resolve,reject)=>{let elem=document.createElement('script');elem.setAttribute('src',base_href+'?!=-style'+component+context+version);let cbres=(e)=>{resolve(e);e.target.removeEventListener('load',cbres)};let cbrej=(e)=>{reject(e);e.target.removeEventListener('error',cbrej)};elem.addEventListener('load',cbres);elem.addEventListener('error',cbrej);document.head.appendChild(elem);});let promises=[controller,template,style];Promise.all(promises).then(()=>{me._resource_cache=me._resource_cache.concat(load);});return promises;}}";
        return $js;
    }
